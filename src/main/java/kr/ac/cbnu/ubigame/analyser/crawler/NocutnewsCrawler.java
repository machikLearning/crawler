...
public class NocutnewsCrawler extends AbsCrawler {
	...
	public String bodyCrawler() throws IOException{/*신문 본문 크롤링*/
		...
		Elements l = GetDocument().select("div[itemprop=articleBody]");
		return l.text();
	}


	public String journalIDSearch() {
		Elements l = GetDocument().select("meta[name=dable:item_id]");
		return l.text();
	}

	public Date timeAdjust() {/*기사게시시간을 yyyymmdd 형식으로 변형시켜주는 함수*/
		Elements div = GetDocument().select("span[id=lblDateLine]");
	...
	public String titleCrawler() {/*기사 제목 크롤링*/
		Elements l = GetDocument().select("div[class=gnb_con]>h2");
		return l.text();
	}
