...
public class SBSCrawler extends AbsCrawler{
	...
	public String bodyCrawler() throws IOException{/*신문 본문 크롤링*/
		...
		Elements l = GetDocument().select("div[class=main_text]");
		return l.text();
	}


	public String journalIDSearch() {
		Elements l = GetDocument().select("meta[itemprop=url]");
		return l.text();
	}

	public Date timeAdjust() {/*기사게시시간을 yyyymmdd 형식으로 변형시켜주는 함수*/
		Elements div = GetDocument().select("span[class=date]>span");
