...
public class AsiaeconomyCrawler extends AbsCrawler {
	...
	public String bodyCrawler() throws IOException{/*신문 본문 크롤링*/
		...
		Elements l = GetDocument().select("div[id = articleBodyContents]");
		return l.text();
	}
	...
	public Date timeAdjust() {/*기사게시시간을 yyyymmdd 형식으로 변형시켜주는 함수*/
		Elements div = GetDocument().select("div[class = real_date]");
	...
	public String titleCrawler() {/*기사 제목 크롤링*/
		Elements l = GetDocument().select("h3[id = articleTitle]");
		return l.text();
