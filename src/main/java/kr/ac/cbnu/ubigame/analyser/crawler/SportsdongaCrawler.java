...
public class SportsdongaCrawler extends AbsCrawler {
	...
	public String bodyCrawler() throws IOException{/*신문 본문 크롤링*/
		...
		Elements l = GetDocument().select("div[class = article_word]");
		return l.text();
	}
	...
	public Date timeAdjust() {/*기사게시시간을 yyyymmdd 형식으로 변형시켜주는 함수*/
		Elements div = GetDocument().select("div[class=article_tit]>p");
	...
	public String titleCrawler() {/*기사 제목 크롤링*/
		Elements l = GetDocument().select("div[class=article_tit] > h3");
		return l.text();
	}
