package kr.ac.cbnu.ubigame.analyser.crawler;

import java.io.IOException;
import java.text.ParseException;
import java.util.Date;

import kr.ac.cbnu.ubigame.analyser.common.crawler.AbsCrawler;

public class FinacialNews extends AbsCrawler {

	public FinacialNews(String targetURL) throws IOException {
		super(targetURL);
		// TODO Auto-generated constructor stub
		this.journal = "파이낸셜뉴스";
	}
	
	public FinacialNews(String url, String searchword, Date finalDate) throws IOException {
		// TODO Auto-generated constructor stub
		super(url,searchword,finalDate);
		this.journal = "파이낸셜뉴스";
	}

	@Override
	protected void initArrayBody() {
		this.arrayBody.add(" div[id=article_content]");
		this.arrayBody.add(" div[class=article_body]");
		this.arrayBody.add(" div[class=news_content]");
		this.arrayBody.add(" div[class=news_text]");
		this.arrayBody.add(" font[class=article]");
		this.arrayBody.add(" div[class=content]");
		this.arrayBody.add("html");
	}

	@Override
	protected void initArrayTitle() {
		this.arrayTitle.add(" div[class=article_head]");
		this.arrayTitle.add("title");

	}

	@Override
	public Date timeAdjust() throws ParseException {
		// TODO Auto-generated method stub
		return null;
	}


}
