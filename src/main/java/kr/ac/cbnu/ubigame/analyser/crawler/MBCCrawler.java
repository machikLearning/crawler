...
public class MBCCrawler extends AbsCrawler {
	...
	public String bodyCrawler() throws IOException{/*신문 본문 크롤링*/
		...
		Elements l = GetDocument().select("section[class = txt]");
		return l.text();
	}


	public String journalIDSearch() {
		Elements l = GetDocument().select("meta[name=nextweb:artId]");
		return l.text();
	}

	public Date timeAdjust() {/*기사게시시간을 yyyymmdd 형식으로 변형시켜주는 함수*/
		Elements div = GetDocument().select("section[class=wrap_time]");
