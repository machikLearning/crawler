...
public class OhmynewsCrawler extends AbsCrawler {
	...
	public String bodyCrawler() throws IOException{/*신문 본문 크롤링*/
		...
		Elements l = GetDocument().select("div[class=article_view]");
		return l.text();
	}
	...
	public Date timeAdjust() {/*기사게시시간을 yyyymmdd 형식으로 변형시켜주는 함수*/
		Elements div = GetDocument().select("div[class=info_data] >div");
	...
	public String titleCrawler() {/*기사 제목 크롤링*/
		Elements l = GetDocument().select("h3[class=tit_subject] > a");
		return l.text();
	}
