...
public class MkCrawler extends AbsCrawler{
	...
	public String bodyCrawler() throws IOException{/*신문 본문 크롤링*/
		...
		Elements l = GetDocument().select("div[class=art_txt]");
		return l.text();
	}
	...
	public Date timeAdjust() {/*기사게시시간을 yyyymmdd 형식으로 변형시켜주는 함수*/
		Elements div = GetDocument().select("div[class=news_title_text] > ul > li[class=lasttime]");
	...
	public String titleCrawler() {/*기사 제목 크롤링*/
		Elements l = GetDocument().select("h1[class=top_title]");
		return l.text();
	}
